  ...
  return new P(function (resolve, reject) {
    request(serverArgs, function (err, res, body) {
      ...
      if (verbose) {
        ...
        console.log('STATUS CODE: %s\n---', utils.prettyjson(res.statusCode));
      }
      if (err) {
        err._headers = res.headers;
      ...
      // Inject a read-only, non-enumerable `_headers[]` and `_statusCode`
      // property in to the response object, `body`.
      Object.defineProperties(body, {
        _headers: {
          enumerable: false,
          writable: false,
          value: sortedJson(res.headers) // HTTP response headers (as Array).
        },
        _statusCode: {
          enumerable: false,
          writable: false,
          value: res.statusCode // HTTP status code (as Number/int).
        }
      });
...
function sortedJson(obj) {
  return JSON.parse(utils.stringify(obj));
}
